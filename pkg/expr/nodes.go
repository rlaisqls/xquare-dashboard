...
func buildCMDNode(rn *rawNode) (*CMDNode, error) {
...
// DSNode is a DPNode that holds a datasources request.
type DSNode struct {
	baseNode
...
// executeDSNodesGrouped groups datasources node queries by the datasources instance, and then sends them
// in a single request with one or more queries to the datasources.
func executeDSNodesGrouped(ctx context.Context, now time.Time, vars mathexp.Vars, s *Service, nodes []*DSNode) {
	type dsKey struct {
		dsType datasources.DataSourceType
	}
	byDS := make(map[dsKey][]*DSNode)
	for _, node := range nodes {
		k := dsKey{dsType: node.datasource.Type}
	...
	for _, nodeGroup := range byDS {
		func() {
			...
			instrument := func(e error, rt string) {
				...
				s.metrics.dsRequests.WithLabelValues(respStatus, fmt.Sprintf("%t", useDataplane), string(firstNode.datasource.Type)).Inc()
			}
...
func (dn *DSNode) Execute(ctx context.Context, now time.Time, _ mathexp.Vars, s *Service) (r mathexp.Results, e error) {
	...
	defer func() {
		...
		logger.Debug("Data source queried", "responseType", responseType)
		useDataplane := strings.HasPrefix(responseType, "dataplane-")
		s.metrics.dsRequests.WithLabelValues(respStatus, fmt.Sprintf("%t", useDataplane), string(dn.datasource.Type)).Inc()
	}()
...
func convertDataFramesToResults(ctx context.Context, frames data.Frames, datasourceType datasources.DataSourceType, s *Service, logger log.Logger) (string, mathexp.Results, error) {
	if len(frames) == 0 {
		return "no-data", mathexp.Results{Values: mathexp.Values{mathexp.NewNoData()}}, nil
	}

	if isAllFrameVectors(datasourceType, frames) { // PrometheusType Specific Handling
	...
	maybeFixerFn := checkIfSeriesNeedToBeFixed(filtered)
...
func isAllFrameVectors(datasourceType datasources.DataSourceType, frames data.Frames) bool {
	if datasourceType != datasources.PrometheusType {
		return false
	}
	allVector := false
...
func checkIfSeriesNeedToBeFixed(frames []*data.Frame) func(series mathexp.Series, valueField *data.Field) {
