...
import (
	...
	"github.com/xquare-dashboard/pkg/tsdb/prometheus/kinds/dataquery"
...
// - go test -benchmem -run=^$ -bench ^BenchmarkExemplarJson$ github.com/xquare-dashboard/pkg/tsdb/prometheus/querydata -memprofile memprofile.out -count 6 | tee old.txt
...
// when memory-profiling this benchmark, these commands are recommended:
// - go test -benchmem -run=^$ -bench ^BenchmarkRangeJson$ github.com/xquare-dashboard/pkg/tsdb/prometheus/querydata -memprofile memprofile.out -count 6 | tee old.txt
// - go tool pprof -http=localhost:6061 memprofile.out
